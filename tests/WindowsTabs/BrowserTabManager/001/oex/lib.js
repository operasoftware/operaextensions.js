...
function createGroup(tabs, properties, container, before) {
    return g[g.length] = container ? container.tabGroups.create(tabs, properties, before) : groups.create(tabs, properties, before);
}
 ...
 * WRONG_ARGUMENTS_ERR === TYPE_MISMATCH_ERR. So that we pass these tests even
 * though we're throwing an old kind exception
 */
...
var assert_throws = function(code, func, description) {
    if (code == 'TYPE_MISMATCH_ERR') {
        try {
            func.call(this);
            assert(false, make_message("assert_throws", description, "${func} did not throw", {
                func : func
            }));
        } catch (e) {
            if (e.code != DOMException.TYPE_MISMATCH_ERR && e.message != 'TYPE_MISMATCH_ERR' && e.message != 'WRONG_ARGUMENTS_ERR') {
...
add_completion_callback(function(tests, status) {
    ...
    if (!resultWindow || resultWindow.closed) {
        resultWindow = opera.extension.windows.create([ {
            url : dataurl
        } ], {
            focused : true
        });
    } else {
        resultWindow.tabs.create({
            url : dataurl,
            focused : true
        });
    }
