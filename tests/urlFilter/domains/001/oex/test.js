opera
	.isReady(function() {
	    ...
	    tests["block"] = async_test("The rule should only block on domains that are included.");
	    ...
	    block
		    .add(
				...
				includeDomains : [
					...
					"invalid\uFFFE\u0000",
					"testsuites.oslo.osa",
					"partial.aliases.t.oslo.osa",
					"aaaaaaaaaabbbbbbbbbbccccccccccddddddddddeeeeeeeeeeffffffffffggg.aaaaaaaaaabbbbbbbbbbccccccccccddddddddddeeeeeeeeeeffffffffffggg.aaaaaaaaaabbbbbbbbbbccccccccccddddddddddeeeeeeeeeeffffffffffggg.aaaaaaaaaabbbbbbbbbbccccccccccddddddddddee.aliases.t.oslo.osa" ]
			    });
	    var expected = [
		    "t",
		    "testsuites.oslo.osa",
		    "aaaaaaaaaabbbbbbbbbbccccccccccddddddddddeeeeeeeeeeffffffffffggg.aaaaaaaaaabbbbbbbbbbccccccccccddddddddddeeeeeeeeeeffffffffffggg.aaaaaaaaaabbbbbbbbbbccccccccccddddddddddeeeeeeeeeeffffffffffggg.aaaaaaaaaabbbbbbbbbbccccccccccddddddddddee.aliases.t.oslo.osa" ];
	    // Longest possible domain name: 253 characters total, up to 63
		...
		if (evt.data.type === "contentblocked") {
			...
			assert_equals(evt.data.url,
				"http://t/resources/images/100x100-navy.png",
				"The correct URL should be blocked.");
			assert_true(expected.indexOf(evt.data.hostname) !== -1,
				"The block should only apply on specified domains. (Location: "
					+ evt.data.location + ")")
		    });
		} else if (evt.data.type === "contentunblocked") {
		    tests["block"].step(function() {
			assert_unreached("Unexpected message recieved: "
		...
		count++;
		if (count === 3) {
		    setTimeout(function() { // Wait in case unexpected events
					    // fire
			tests["block"].done();
		    }, 10);
		}
