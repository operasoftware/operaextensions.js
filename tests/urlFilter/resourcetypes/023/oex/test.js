opera.isReady({
    ...
    tests["block"] = async_test("Blocking by resource type: subdocument (frame).");
    block.add("*fail.html*", {resources: types("subdocument")});
    ...
    opera.extension.onmessage = function(evt) {
    	...
    	if (evt.data.type === "contentblocked") {
    		tests["block"].step(function(){
    			...
    			assert_equals(evt.data.tagName.toUpperCase(), "FRAME", "The correct element should be blocked.");
    		});
    ...
    var data = "<!DOCTYPE html><frameset rows='*,*'><frame src='http://t/resources/fail.html'><frame></frameset>";
