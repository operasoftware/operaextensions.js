...
this.length){f=this.length;for(i=b;f<i;f++)delete this[f]}}};r.prototype=Object.create(n.prototype);r.prototype.create=function(a,d){if(d&&!(d instanceof h))throw new p("Type mismatch","Could not create BrowserTab object. 'before' attribute provided is invalid.");if(this._parent&&!0===this._parent.closed)throw new p("Invalid state","Parent window of the current BrowserTab object is in the closed state and therefore is invalid.");a=a||{};a.closed&&delete a.closed;var b=new h(a,this._parent||g.windows.getLastFocused()),
a=b.properties;b.properties.index=a.index=b._windowParent.tabs.length;a.windowId=this._parent?this._parent.properties.id:g.windows.getLastFocused().properties.id;if(d&&d instanceof h){if(!0===d.closed)throw new p("Invalid state","'before' BrowserTab object is in the closed state and therefore is invalid.");if(d._windowParent&&!0===d._windowParent.closed)throw new p("Invalid state","Parent window of 'before' BrowserTab object is in the closed state and therefore is invalid.");a.windowId=d._windowParent?
d._windowParent.properties.id:a.windowId;a.index=d.position}this===g.tabs&&(b.properties.index=a.index=g.windows.getLastFocused().tabs.length);b._windowParent.tabs.addTab(b,b.properties.index);g.tabs.addTab(b);this.enqueue(chrome.tabs.create,a,function(a){for(var d in a)d!="url"&&(b.properties[d]=a[d]);if(this!==g.tabs){this.removeTab(b);this.addTab(b,b.properties.index)}b.resolve(true);this.dispatchEvent(new j("create",{tab:b,prevWindow:b._windowParent,prevTabGroup:null,prevPosition:NaN}));this.dequeue()}.bind(this));
return b};r.prototype.getAll=function(){for(var a=[],d=0,b=this.length;d<b;d++)a[d]=this[d];return a};r.prototype.getSelected=function(){return this._lastFocusedTab||this[0]};r.prototype.getFocused=r.prototype.getSelected;r.prototype.close=function(a){if(!a||!(a instanceof h))throw new p("Type mismatch","Expected browserTab argument to be of type BrowserTab.");a.close()};var N=function(){function a(a,b){if(!this._blackList[a]){for(var c=-1,e=0,f=this._allTabs.length;e<f;e++)if(this._allTabs[e].properties.id==
a){c=e;break}if(!(0>c)&&(c=this._allTabs[c]))c._oldWindowParent?c._oldWindowParent.tabs.removeTab(c):c._windowParent.tabs.removeTab(c),c._windowParent.tabs.addTab(c,b.toIndex),c.dispatchEvent(new j("move",{tab:c,prevWindow:c._oldWindowParent,prevTabGroup:null,prevPosition:c._oldIndex})),this.dispatchEvent(new j("move",{tab:c,prevWindow:c._oldWindowParent,prevTabGroup:null,prevPosition:c._oldIndex})),void 0!==c._oldWindowParent&&delete c._oldWindowParent,void 0!==c._oldIndex&&delete c._oldIndex}}r.call(this);
this._blackList={};this._allTabs=[];chrome.tabs.onCreated.addListener(function(a){l.setTimeout(function(){if(!this._blackList[a.id]){for(var b=-1,c=0,e=this._allTabs.length;c<e;c++){if(this._allTabs[c].rewriteUrl&&this._allTabs[c].properties.url==a.url){if(this._allTabs[c]._windowParent&&this._allTabs[c]._windowParent.properties.id!==a.windowId)for(var f=0,e=g.windows.length;f<e;f++)g.windows[f].properties.id==a.windowId&&(this._allTabs[c]._windowParent.tabs.removeTab(this._allTabs[c]),this._allTabs[c].properties.index=
this._allTabs[c]._windowParent.tabs.length,this._allTabs[c]._windowParent=g.windows[f],this._allTabs[c].properties.windowId=a.windowId,g.windows[f].tabs.addTab(this._allTabs[c],this._allTabs[c].properties.index));for(f in a)"url"==f||"windowId"==f||(this._allTabs[c].properties[f]=a[f]);this._allTabs[c].url=this._allTabs[c].rewriteUrl;delete this._allTabs[c].rewriteUrl;return}if(this._allTabs[c].properties.id==a.id){b=c;break}}if(0>b){for(var i,f=u.extension.windows,c=0,e=f.length;c<e;c++)if(f[c].properties.id==
a.windowId){i=f[c];break}i||(i=g.windows.getLastFocused());c=new h(a,i);c._windowParent.tabs.addTab(c,c.properties.index);this.addTab(c);c.resolve(!0)}else{for(c in a)this[b].properties[c]=a[c];c=this[b]}c._windowParent.tabs.dispatchEvent(new j("create",{tab:c,prevWindow:c._windowParent,prevTabGroup:null,prevPosition:NaN}));this.dispatchEvent(new j("create",{tab:c,prevWindow:c._windowParent,prevTabGroup:null,prevPosition:NaN}))}}.bind(this),200)}.bind(this));chrome.tabs.onRemoved.addListener(function(a){if(!this._blackList[a]){for(var b=
-1,c=0,e=this._allTabs.length;c<e;c++)if(this._allTabs[c].properties.id==a){b=c;break}if(!(0>b)){a=this._allTabs[b];c=a._oldWindowParent;b=a._oldIndex||a.properties.index;if(!c&&void 0!==a._windowParent&&null!==a._windowParent){a.properties.closed=!0;a._windowParent.tabs.removeTab(a);this.removeTab(a);c=0;for(e=a._windowParent.tabs.length;c<e;c++)!0==a._windowParent.tabs[c].properties.active?a._windowParent.tabs[c].focus():a._windowParent.tabs[c].properties.active=!1;a.properties.index=NaN;c=a._windowParent;
a._windowParent=null}a.dispatchEvent(new j("close",{tab:a,prevWindow:c,prevTabGroup:null,prevPosition:b}));c&&c.tabs.dispatchEvent(new j("close",{tab:a,prevWindow:c,prevTabGroup:null,prevPosition:b}));this.dispatchEvent(new j("close",{tab:a,prevWindow:c,prevTabGroup:null,prevPosition:b}))}}}.bind(this));chrome.tabs.onUpdated.addListener(function(a,b,c){for(var b=-1,e=0,f=this._allTabs.length;e<f;e++)if(this._allTabs[e].properties.id==a){b=e;break}if(!(0>b)){var a=this._allTabs[b],g;for(g in c)"id"==
g||"windowId"==g||(a.properties[g]=c[g]);!0==a.properties.active&&(a._windowParent&&a._windowParent.tabs._lastFocusedTab!==a)&&a.focus()}}.bind(this));chrome.tabs.onMoved.addListener(a.bind(this));chrome.tabs.onAttached.addListener(function(d,b){for(var c=-1,e=0,f=this._allTabs.length;e<f;e++)if(this._allTabs[e].properties.id==d){c=e;break}if(!(0>c)){var i=this._allTabs[c];i._oldWindowParent&&i._oldWindowParent.tabs.removeTab(i);l.setTimeout(function(){for(var c=0,e=g.windows.length;c<e;c++)if(g.windows[c].properties.id==
